...
// Global Symbol Search Proxy (Yahoo Finance Autocomplete + NSE)
    ...
    if (response.ok) {
      ...
      const formatted = quotes
        ...
        .map(q => {
          ...
          const exch = isNSE ? 'NSE' : (isBSE ? 'BSE' : (q.exchDisp || q.exchange || 'GLOBAL'));
          const symbolStr = (isNSE || isBSE) ? `${exch}:${rawTicker}` : q.symbol;
          ...
          return {
            ...
            code: rawTicker,
            exchange: exch,
            group: (isNSE || isBSE) ? (q.quoteType === 'INDEX' ? 'Indices' : 'Stocks') : (q.quoteType === 'INDEX' ? 'Global Indices' : 'Global'),
